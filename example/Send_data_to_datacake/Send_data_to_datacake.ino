...
#define device "851d2744-1920-4907-a9b4-5cb568c75b3e" //Your Serial number
...
void lora_receive_task()
{
    ...
    if (state == ERR_NONE)
    {
        ...
        json2url(str, url);
        sendData("AT+HTTPINIT", 1000);
        sendData("AT+HTTPPARA=\"URL\",\"https://api.datacake.co/integrations/api/d4ad8b1a-215e-402b-93a3-a6c110eed492/\"", 1000);  //Your HTTP endpoint URL
        sendData("AT+HTTPDATA=72,1000", 3000);  //send data to post, the length is 85 bytes
        sendData(url, 1000);
...
void json2url(String input, char *url)
{
    ...
    sprintf(url,"{\"device\":\"%s\",\"ADC\":%.1f,\"bat\":%.1f}",device,ADC,bat); //{"device":"e570efbf-cca7-4fe8-8670-909b1e01d689","temperature":25.6,"humidity":47.7}
    USBSerial.println(url);
}
